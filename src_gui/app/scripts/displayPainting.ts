...
export async function displayPainting(paintingConfig) {
  const basepath = "data/json/";
  try {
    const promises = {
      json2: paintingConfig.json2
        ? $.getJSON(basepath + paintingConfig.json2)
        : null,
      grabcut2: paintingConfig.grabcut2
        ? $.getJSON(basepath + paintingConfig.grabcut2)
        : null,
      grabcut: paintingConfig.grabcut
        ? $.getJSON(basepath + paintingConfig.grabcut)
        : null,
      imofaColor: paintingConfig.imofaColor
        ? $.getJSON(basepath + paintingConfig.imofaColor)
        : null,
      imofa2Color: paintingConfig.imofa2Color
        ? $.getJSON(basepath + paintingConfig.imofa2Color)
        : null
    };
    ...
    const imofaToJQ = imofaJson => {
      if (imofaJson == null) {
        ...
        imofaJson
          ...
          .forEach(wrgb => {
            imofaHTML += `<tr><td>${(wrgb[0] * 100).toFixed(
              2
            )}%</td><td><input type="color" value="#${toHex(wrgb[1])}${toHex(
              wrgb[2]
            )}${toHex(wrgb[3])}"></input></td>`;
          });
        return imofaHTML + "</table>";
      }
    };

    const jDispContent = $(`<div class="tabcontent"><h2>${
      json2.title
    }</h2><canvas id="myCanvas" width="1200" height="1200" style="display:none"/>
    <table style="margin-left:auto; margin-right:auto;border-spacing: 10px; border-collapse: separate;">
        ...
        <tr>
            <td colspan="2" style="width:24%"><img style="padding=2em" id="original" src="data/jpg/${
              json2.Filename
            }"></img></td>
