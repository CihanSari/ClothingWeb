function kmeans(data, config) {
  ...
  var centroids = [];
  //const indices = getRandomIndices(points.length, k);
  for (var i = 0; i < k; i++) {
    const kPerc = i/k;
    const iPerc = Math.floor(kPerc * points.length);
    centroids.push(new Centroid(points[iPerc].location(), i));
  };
...
function getRandomIndices(length, count) {
  ...
  for (let i = 0; i < count; i += 1) {
    let bAssignmentComplete = false;
    while (!bAssignmentComplete) {
      const val = getRandomInt(0, length);
      let bMoveOn = false;
